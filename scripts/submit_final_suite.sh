...
echo "Submitted MATH-500 final protocol: $MATH500_JOB_ID"

GSM8K_JOB_ID="$(submit_protocol repairable_diffusion/configs/final/protocol_gsm8k_final.yaml)"
echo "Submitted GSM8K final protocol: $GSM8K_JOB_ID"

AGGREGATE_JOB_ID="$(
  sbatch --parsable \
    --dependency="afterok:${MATH500_JOB_ID}:${GSM8K_JOB_ID}" \
    --job-name=repair_aggregate \
    --output="$ROOT_DIR/logs/final_aggregate_%j.out" \
    --error="$ROOT_DIR/logs/final_aggregate_%j.err" \
    --wrap="cd '$ROOT_DIR' && bash scripts/build_final_report.sh"
)"
echo "Submitted aggregate job: $AGGREGATE_JOB_ID"
echo "All jobs chained successfully."
