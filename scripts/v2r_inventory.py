...
import argparse, concurrent.futures, datetime as dt, json, os, shlex, subprocess, tempfile
...
PROTECTED_JOBS = ['50668','50669','50738','50752','50753','50754','50923','50924','49256']
...
def collect(output):
    ...
    inventory={'schema_version':'v2r.inventory.1','timestamp':now,'critical_usage_fraction':0.95,'minimum_safety_margin_bytes':50*1024**3,'servers':servers,
      'slurm_jobs':run(['squeue','-h','-o','%i|%u|%T|%M|%l|%D|%N|%b|%j']),
      'protected_jobs':run(['sacct','-n','-P','-j',','.join(PROTECTED_JOBS),'--format=JobID,State,Elapsed,Timelimit,NodeList,AllocTRES,ExitCode']),
