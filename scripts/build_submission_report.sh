...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/results/submission_reports}"
...
ROBUSTNESS_REPORT="${ROBUSTNESS_REPORT:-$ROOT_DIR/results/generated_configs/protocol_math500_submission_robustness_report.json}"

missing=()
for path in "$MATH500_REPORT" "$GSM8K_REPORT" "$ROBUSTNESS_REPORT"; do
  if [[ ! -f "$path" ]]; then
    missing+=("$path")
  fi
done
...
if (( ${#missing[@]} > 0 )); then
  echo "missing protocol report(s) required for build_submission_report.sh:" >&2
  for path in "${missing[@]}"; do
    echo "  - $path" >&2
  done
  echo "set MATH500_REPORT/GSM8K_REPORT/ROBUSTNESS_REPORT to alternate report paths or complete the missing protocol runs first." >&2
  exit 1
fi
...
"$PYTHON_BIN" -m repairable_diffusion.src.analysis.aggregate_runs \
  ...
  --protocol-report "$ROBUSTNESS_REPORT" \
