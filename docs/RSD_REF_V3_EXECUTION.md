...
- Design freeze: `status/rsd_ref_v3/design_freeze.json`.
- Readiness: `status/rsd_ref_v3/execution_readiness.json`.
...
The design audit must pass before any source-native bank or confirmatory subset is created. `--mode readiness` is expected to fail while `storage_plan.json` is `STORAGE_NOT_RESERVED`.
...
1. generate the full LLaDA source-native MATH bank, one trajectory per item;
2. generate the full LLaDA source-native GSM8K bank, one trajectory per item;
3. seal the final failure pool and materialize deterministic core/temporal/mechanism/harm subsets;
4. execute primary controls and confirmation in the frozen order;
5. retain raw counts, denominators, branch registries, NFE, evaluator identity, and storage seals.
